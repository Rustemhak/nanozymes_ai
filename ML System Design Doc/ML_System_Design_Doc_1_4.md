...
Для предикта текст статьи делится на фрагменты длиной N символов, фрагменты передаются модели multilingual-e5-large для построения эмбеддингов, строится той же моделью эмбеддинг запроса пользователя, выбирается k фрагментов текста, косинусное расстояние между эмбеддингами которых и эмбеддингом запроса наименьшее, к каждому из k, фрагментов добавляется q фрагментов до и столько же после, k таких наборов данных в формате JSON по API передаются модели OpenAI. 
Возвращается ответ в формате JSON или .xml для отображения в области распознанного текста
