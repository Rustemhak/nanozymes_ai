...
Текущий этап проектирования ML системы - исследование (первое проблемное интервью назначено на 21.09.2023). Все внесенные в шаблон данные - гипотезы, которые планируется проверить и/или дополнить на интервью.

### 1. Цели и предпосылки
#### 1.1. Зачем идем в разработку продукта?
- Бизнес-цель `Product Owner`  Публичная база знаний по нанозимам. Ускорение пополнения базы знаний за счет распознавания данных в статьях
- Почему станет лучше, чем сейчас, от использования ML `Product Owner` & `Data Scientist` X статей можно просмотреть вручную, +Y% можно обработать с помощью ML - обсуждаем с экспертом
- Что будем считать успехом итерации с точки зрения бизнеса `Product Owner` Распознавание параметров синтеза с точностью N%
#### 1.2. Бизнес-требования и ограничения
- Краткое описание БТ и ссылки на детальные документы с бизнес-требованиями `Product Owner`   Парсинг текста, классификация статей, Извлечение параметров, Удобный личный кабинет пользователя
- Бизнес-ограничения `Product Owner`  Есть юридические ограничения на открытое размещение данных, на которых обучалась модель
...
- На закрытие каких БТ подписываемся в данной итерации `Data Scientist`   ML-Research: какие наборы моделей/правил etc. дадут наибольшую точность распознавания на тренировочном датасете
- Что не будет закрыто `Data Scientist`  Удобный личный кабинет пользователя
- Описание результата с точки зрения качества кода и воспроизводимости решения `Data Scientist`  
- Описание планируемого технического долга (что оставляем для дальнейшей продуктивизации) `Data Scientist` Личный кабинет пользователя
